...
CSV_FILE=${CSV_FILE:-cpu_bidirectional_ring_small_vector_results.csv}
LOG_DIR=${LOG_DIR:-logs_cpu_bidirectional_ring_small_vector}
mkdir -p "${LOG_DIR}"

TRAIN_SCRIPT=${TRAIN_SCRIPT:-train_cpu_ddp_custom.py}
STEPS=${STEPS:-100}
BATCH_SIZE=${BATCH_SIZE:-16}
THREADS=${THREADS:-1}
BUCKET_CAP_MB=${BUCKET_CAP_MB:-512}
REPEATS=${REPEATS:-5}
NPROC_7=${NPROC_7:-7}
NUM_CLASSES=2

if (( NPROC_7 % 2 == 0 )); then
  echo "ERROR: bidirectional-ring requires an odd world_size. Use NPROC_7=7."
  exit 1
fi

# With hidden_dim=1, num_layers=1, num_classes=2:
# total_params = input_dim + 5, so the gradient vector is total_params * 4 bytes.
# Format: "input_dim hidden_dim num_layers model_label target_vector_size_bytes"
MODEL_CONFIGS=(
  "251 1 1 1KiB 1024"
  "507 1 1 2KiB 2048"
  "1019 1 1 4KiB 4096"
...
rm -f "${CSV_FILE}"
echo "algo,world_size,input_dim,hidden_dim,num_classes,num_layers,model_label,target_vector_size_bytes,total_params,grad_size_bytes,grad_size_mib,steps,batch_size,bucket_cap_mb,threads,repeat_id,total_training_time_sec,avg_step_time_ms,parameter_max_diff,torchrun_status,accepted,log_file" > "${CSV_FILE}"
...
run_one() {
  ...
  local safe_algo="${algo//-/_}"
  local log_file="${LOG_DIR}/${safe_algo}_n${nproc}_${model_label}_rep${repeat_id}.log"
...
PY
  ...
  echo "${algo},${nproc},${input_dim},${hidden_dim},${NUM_CLASSES},${num_layers},${model_label},${target_vector_size_bytes},${total_params},${grad_size_bytes},${grad_size_mib},${STEPS},${BATCH_SIZE},${BUCKET_CAP_MB},${THREADS},${repeat_id},${total_time},${avg_step},${max_diff},${run_status},${accepted},${log_file}" >> "${CSV_FILE}"
...
echo "All CPU bidirectional-ring small-vector experiments finished."
